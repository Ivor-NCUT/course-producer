...
知识库/                                 方法论、模板、词典、原创知识原子
   ↓
tools/course-project.mjs               确定性状态机、材料指纹和完成度验证
   ↓
...
| `course-pilot-iteration` | 用真实试讲证据验证和迭代课程 | 课程版本、作业、访谈与行为记录 | 试讲计划、反馈、修订决定、结果回填 |
| `course-lark-delivery` | 写回飞书并建立评论协作 | 最终稿、审校项、目标 URL | 飞书文档、评论、回读证据 |
| `course-poster-planning` | 提炼海报策略与原型 | 完整课程、销售信息 | 海报方案、可编辑画板 |
| `course-aesthetic-alignment` | 管理反馈证据与安全偏好演化 | 改稿、满意/否定、A/B 选择 | 项目偏好档案、验证结论 |
...
试讲迭代是可选原子能力，不加入核心 `state.json` 阶段列表。这样旧项目可以无迁移继续恢复；用户要求内测、提供学员反馈或验证教学效果时，主路由单独调用它，并把证据写入 `artifacts/pilot/`。
...
知识原子必须来自本项目明确列出的四个源 Skill 或用户后续提供且允许沉淀的材料。每条原子至少包含 `id`、`knowledge`、`type`、`topics`、`source` 和 `confidence`。
...
| `dbs-goal`、`dbs-good-question`、`dbs-knowledge`、`dbs-content-system`、`dbs-learning`、`dbs-decision`、`dbs-deconstruct`、`dbs-logic-continuity`、`dbs-ai-check` | 研究其处理的通用问题，在课程定位、访谈、材料治理、知识萃取、试讲迭代与审校中原创设计；检测到已安装时可作为额外复核，不要求安装 |
